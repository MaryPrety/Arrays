...
class PlanetSimulationApp:
    ...
    def simulate_planetary_system(self):
        ...
        radii = np.random.uniform(0.5, 1.5, num_planets)
        ...
        def update(frame):
            ...
            for i, (eccentricity_entry, mass_entry, orbit_type, color_label, shape_combo, _) in enumerate(self.planet_entries):
                ...
                if orbit_type.get() == "эллиптическая":
                    radius = radii[i] * (1 + ecc * np.cos(angles[i] + frame * 0.05))
                else:
                    radius = radii[i]
                planets[i, 0] = radius * np.cos(angles[i] + frame * 0.05)
                planets[i, 1] = radius * np.sin(angles[i] + frame * 0.05)
